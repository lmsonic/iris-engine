...
impl EguiRenderer {
    ...
    pub fn register_texture(&mut self, texture: &mut Texture, device: &wgpu::Device) {
        texture.egui_id.get_or_insert_with(|| {
            self.renderer
                .register_native_texture(device, &texture.view, wgpu::FilterMode::Linear)
        });
    }
